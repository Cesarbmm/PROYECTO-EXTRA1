...
public class BTreeRenderer extends JFrame {
    ...
    public final static int APP_WIDTH = 1420;
    ...
    public BTreeRenderer() {
        super("Bplus Renderer");
        mTreeTest = new BTreeTest();
        mBuf = new StringBuilder();
        mObjLists = new Object[TREE_HEIGHT];
        mSearchKeyBt = new JButton("Buscar");
        mAddBt = new JButton("Añadir");
        mRemoveBt = new JButton("Eliminar");
        mAddMoreBt = new JButton("Incrementar en 1");
        mRemoveMoreBt = new JButton("Decrementar en 1");
        mClearBt = new JButton("Limpiar");
        mListBt = new JButton("Mostrar elementos");
        mSaveBt = new JButton("Guardar");
        mText = new JTextField("");
        mOutputConsole = new JTextArea(10, 17);
    ...
    deleteKey(in - 1); // Eliminar la clave anterior
    deleteKey(in);     // Eliminar la clave actual
    mText.setText((in - 1) + ""); // Actualizar el campo de texto al valor anterior
    render(); // Forzar la actualización del renderizador
}
*/
/* 
public void removeMoreButtonPressed() {
    ...
    if (in == null) {
        in = 0; // Si es null, eliminar el siguiente valor
        deleteKey(in + 1);
    } else {
        if (in > 0) {
            deleteKey(in - 1); // Eliminar la clave anterior
    ...
    mText.setText((in - 1) + ""); // Actualizar el campo de texto
    render(); // Forzar la actualización del renderizador
}
*/
    ...
    try {
        String strSavedFile = SimpleFileWriter.saveToFile(mOutputConsole.getText());
        mOutputConsole.setText("Successfully saved console text to file = " + strSavedFile);
    } catch (IOException ioex) {
        mOutputConsole.setText("Error: failed to save to file msg = " + ioex.getMessage());
    ...
    */
    ...
    public void render() {
        ...
        List<Object> cObjList = new ArrayList<Object>();
        List<Object> tempObjList;
        List<Object> leafNodes = new ArrayList<Object>(); // Track leaf nodes
        ...
        try {
            ...
            for (int i = 0; i < mObjLists.length; ++i) {
                ...
                // Check if this is the leaf level (next level is null or empty)
                boolean isLeafLevel = (i + 1 >= mObjLists.length || mObjLists[i + 1] == null);
                ...
                for (KeyData keyData : objList) {
                    ...
                    // If this is a leaf node, add it to our leaf nodes list
                    if (isLeafLevel) {
                        leafNodes.add(gObj);
                    }
                ...
                if (i > 0) {
                    // Connect the nodes (parent-child connections)
                    List<KeyData> keyList = (List<KeyData>)mObjLists[i - 1];
                    int j = 0, k = 0;
                    for (Object pObj : pObjList) {
                        KeyData keyData = keyList.get(j);
            ...
            for (int i = 0; i < leafNodes.size() - 1; i++) {
                Object sourceNode = leafNodes.get(i);
                Object targetNode = leafNodes.get(i + 1);
                // Create a different style for leaf connections (e.g., dashed line)
                mGraph.insertEdge(parent, null, "", sourceNode, targetNode, "dashed=1;strokeColor=#999999");
            }
        }
        finally {
            mGraph.getModel().endUpdate();
        ...
        getContentPane().add(hBox, BorderLayout.NORTH);
        getContentPane().add(graphComponent, BorderLayout.CENTER);
        getContentPane().add(new JScrollPane(mOutputConsole), BorderLayout.WEST);
        revalidate();
    }
...
//INICIO ORIGINAL
    ...
    //FIN CAMBIO 2 NO FUNCIONA MUCHO
    //CAMBIO 3
    /*
    private void generateGraphObject(BTNode<Integer, String> treeNode, int nLevel) throws BTException {
        if ((treeNode == null) || (treeNode.mCurrentKeyNum == 0)) {
        ...
        for (int i = 0; i < currentKeyNum; ++i) {
            keyVal = treeNode.mKeys[i];
            if (keyVal != null) { // Verificar si keyVal es null
                if (i > 0) {
                    mBuf.append(" | ");
                }
                mBuf.append(keyVal.mKey);
    ...
    */
    ...
    private void generateGraphObject(BTNode<Integer, String> treeNode, int nLevel) throws BTException {
    if (treeNode == null) {
        return; // Nodo nulo, no lo renderices
    }
    ...
    mBuf.setLength(0);
    if (currentKeyNum == 0) {
        mBuf.append("Vacío"); // Mostrar "Vacío" para nodos sin claves
    } else {
        for (int i = 0; i < currentKeyNum; ++i) {
            keyVal = treeNode.mKeys[i];
            if (keyVal != null) { // Verificar si la clave es null
                if (i > 0) {
                    mBuf.append(" |VACIO ");
                }
                mBuf.append(keyVal.mKey);
    ...
    */
    ...
    // Asegúrate de que el índice de nivel no se salga de los límites
    if (nLevel >= mObjLists.length) {
        throw new BTException("El nivel excede el tamaño de mObjLists");
    }
    ...
    mBuf.setLength(0);  // Limpiar el StringBuilder antes de usarlo
    ...
    if (currentKeyNum == 0) {
        ...
        for (int i = 0; i < currentKeyNum; ++i) {
            keyVal = treeNode.mKeys[i];
            if (keyVal != null && keyVal.mKey != null) { // Asegurarse de que la clave no sea null
                if (i > 0) {
                    mBuf.append(" | ");
                }
                mBuf.append(keyVal.mKey);
    ...
    if (treeNode.mIsLeaf) {
        return; // Si es un nodo hoja, no tiene hijos, terminar aquí
    }

    // Renderizar los hijos
    ++nLevel; // Aumentar el nivel para los nodos hijos
    for (int i = 0; i < currentKeyNum + 1; ++i) {
        generateGraphObject(treeNode.mChildren[i], nLevel);
    }
}
